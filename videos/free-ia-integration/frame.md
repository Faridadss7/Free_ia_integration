...
## Fonts — staged local WOFF2 (copy this @font-face block verbatim into each frame's `<template>`)

The preset fonts are staged in `assets/fonts/` (project-root-relative). Every frame that names Inter or JetBrains Mono MUST include this exact block inside its `<template>` (`src` paths are project-root-relative — never `../`):
...
@font-face { font-family: "JetBrains Mono"; font-weight: 500; font-style: normal; font-display: block; src: url("assets/fonts/JetBrainsMono-700.woff2") format("woff2"); }
```

Only these two families ship as files (weights 400/700; the mono 500 in the type ramp maps to the 700 file). Name no other font. Author display at weight 400; italic is the browser-synthesized slant.
